...
public class QualificationController {
  ...
  private void initialize() {
    ...
    // nuit, et associer chaque colonne à la propriété du modèle correspondante via
    // setCellValueFactory.
              ...
              if (newV == null)
                ...
                labelSelection.setText(
                    "Séquence "
                        + newV.getHorodatage()
                        + " - "
                        + String.format("%.1f kHz", newV.getFrequenceDominanteKHz()));
              }
            });
  ...
  private void ecouter() {
    ...
    Sequence sequenceSel = tableView.getSelectionModel().getSelectedItem();
    if (sequenceSel != null) sequenceSel.setStatut("Écoutée");
  ...
  private void enregistrerVerdict() {
    ...
    // dans le modèle via nuit.setVerdictGlobal(...). Ne rien faire si aucun verdict
    // n'est sélectionné.
